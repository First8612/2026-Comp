...
public class Shooter extends SubsystemBase {
    ...
    public void periodic() {
        ...
        if (isAiming) {
            ...
            for (var i = 1; i < shootCalc.length; i++) {
                if (distance < shootCalc[i].getDist() && !hasAimed) {
                    lerpAmount = (distance - shootCalc[i - 1].getDist())
                            / (shootCalc[i].getDist() - shootCalc[i - 1].getDist());
        ...
        // hoodMotor.setControl(new
        // PositionVoltage(0).withSlot(0).withPosition(currHoodGoal));
        if (flywheelSpeedGoal == 0) {
        ...
        // SmartDashboard.putNumber("Shooter/hoodActual",
